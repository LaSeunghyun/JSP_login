...
<title>커뮤니티 - 글보기</title>
	...
	function likebtn(){
		...
		xhr.open('GET', './b_like.jsp?b_idx=<%=b_idx%>', true);
		xhr.send();
	}
	</script>
</head>
<body>
	...
	<table border="1" width="800">
		...
		<tr>
			...
			%>
			<input type="button" value="좋아요" id="likebtn" onclick="likebtn()"> | <input type="button" value="리스트" onclick="location.href='./list.jsp'">
		</tr>
	</table>
	<hr/>
	<form method="post" action="reply_ok.jsp">
		<input type="hidden" name="b_idx" value="<%=b_idx%>">
	<p><%=session.getAttribute("userid")%> : <input type="text" size="40" name="r_content"> <input type="submit" value="확인"> 
	</p></form>
	<hr/>
	<%
	...
	<%
			if(r_userid.equals(session.getAttribute("userid"))){
	%>
			<input type="button" value="삭제" onclick="location.href='./r_del.jsp?r_idx=<%=r_idx%>&b_idx=<%=b_idx%>'">
	<%
			}
		}
	%>
